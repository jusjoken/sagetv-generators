...
<component>
    ...
    ${ctx.css("https://fonts.googleapis.com/icon?family=Material+Icons")}
    ${ctx.css("mdl/material.min.css")}
    ${ctx.css("lib/animate.min.css")}
    ${ctx.css("app.css")}

    ${ctx.jsLazy("mdl/material.min.js")}
    ...
    ${ctx.require("STVApp","js/STVApp.js")}
    ${ctx.require("STVObject","js/STVObject.js")}
        ...
        <div class="mdl-layout mdl-js-layout ${fixedDrawer?then("mdl-layout--fixed-drawer","mdl-layout--drawer")} ${fixedHeader?then("mdl-layout--fixed-header","mdl-layout--header")}">
            ...
            <script language='JavaScript'>
                ...
                require([${funcs}], function(${vars}) {
                    ...
                    var _appData = STVObject.manage({});
                    _appData.setChangeListener('messageCount', function(oldVal, newVal) {
                        console.log("CHANGED:", newVal);
                        $('#message-count').attr('data-badge', newVal).show()
                    });
